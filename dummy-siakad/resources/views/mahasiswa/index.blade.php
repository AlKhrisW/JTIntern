...
@section('content')
    <div class="mahasiswa-page">
        ...
        <div class="page-header d-flex align-items-center justify-content-between mb-4">
            <h4 class="page-title mb-0">Manajemen Mahasiswa</h4>
        ...
        <div class="filter-stats-bar d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
            ...
            <div class="stat-item text-end">
                ...
                <div class="stat-value text-primary fw-bold">{{ $totalMahasiswa ?? 0 }}</div>
            </div>
        </div>
        ...
        <div class="table-card">
            <div class="table-responsive">
                <table id="tableMahasiswa" class="table table-hover align-middle mb-0">
                    <thead>
                        <tr>
                            <th>NIM</th>
                    ...
                    <tbody>
                        @forelse($mahasiswas as $mhs)
                            <tr>
                                <td class="fw-medium text-muted">{{ $mhs->nim }}</td>
                                <td class="fw-semibold">{{ $mhs->nama_mahasiswa }}</td>
                                <td>
                                    <span
                                        class="badge badge-prodi">{{ $mhs->programStudi->nama_program_studi ?? '-' }}</span>
                                </td>
                                <td class="text-end">
                                    <div class="d-flex justify-content-end gap-2">
                                        <button class="btn-icon btn-show" data-id="{{ $mhs->nim }}" title="Detail">
                                            <i class="bi bi-eye"></i>
                                        </button>
                                        <button class="btn-icon btn-edit" data-id="{{ $mhs->nim }}" title="Edit">
                                            <i class="bi bi-pencil"></i>
                                        </button>
                                        <button class="btn-icon btn-delete" data-id="{{ $mhs->nim }}"
                                            data-nama="{{ $mhs->nama_mahasiswa }}" title="Hapus">
            ...
            {{-- Pagination --}}
            @if (isset($mahasiswas) && method_exists($mahasiswas, 'links'))
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('btnTambah').addEventListener('click', function() {
                loadModal('{{ route('admin.mahasiswa.create_ajax') }}');
            });

            // Tombol Show, Edit, Delete
            document.querySelectorAll('.btn-show').forEach(btn => {
                btn.addEventListener('click', function() {
                    loadModal('{{ url('admin/mahasiswa/show_ajax') }}/' + this.dataset.id);
                });
            });

            document.querySelectorAll('.btn-edit').forEach(btn => {
                btn.addEventListener('click', function() {
                    loadModal('{{ url('admin/mahasiswa/edit_ajax') }}/' + this.dataset.id);
                });
            });

            document.querySelectorAll('.btn-delete').forEach(btn => {
                btn.addEventListener('click', function() {
                    loadModal('{{ url('admin/mahasiswa/delete_ajax') }}/' + this.dataset.id);
                });
            });
